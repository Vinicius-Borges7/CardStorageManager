...
export default class Vanguard{
    ...
    async new(){
        ...
        const res = new REST("", body);
        return await res.post();
    }

    static async get(){
        let field = Field.yugioh();
        const req = new REST('Get', {'id': ''});
                ...
                `<div class="col-md-3 mb-4" id="card${e.id}">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <p class="card-text">Card's Category: ${e.gauge}</p>
                        ...
                        <button id="btnDel${e.id}" onClick='delete(${e.id})'>Delete Card</button>
                    </div>
                </div>`
            );
        });
