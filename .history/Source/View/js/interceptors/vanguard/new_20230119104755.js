...
export default async function newVanguard(){
    ...
    if(Inputs.voidCheck(inputs) == true){
        ...
        Field.response("vanguResponseField").innerHTML = "";
        const res = await card.new();

        Field.response("vanguResponseField").innerHTML = res;
    } else {
        Field.response("vanguResponseField").innerHTML = "";
        Field.response("vanguResponseField").innerHTML = "all fields are obrigatory";
    }
}