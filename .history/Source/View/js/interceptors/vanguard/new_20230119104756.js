...
export default async function newVanguard(){
    ...
    if(Inputs.voidCheck(inputs) == true){
        ...
        Field.response("vanguarResponseField").innerHTML = "";
        const res = await card.new();

        Field.response("vanguarResponseField").innerHTML = res;
    } else {
        Field.response("vanguarResponseField").innerHTML = "";
        Field.response("vanguarResponseField").innerHTML = "all fields are obrigatory";
    }
}